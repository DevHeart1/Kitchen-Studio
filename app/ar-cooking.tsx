...
// import { Text as Text3D, Float, Html } from "@react-three/drei";
...
import { useInventory } from "@/contexts/InventoryContext"; // Added import
...
// --- 3D Components (Commented out due to dependency issues) ---
/*
...
export default function ARCookingScreen() {
  ...
  const passedRecipe = useMemo(() => {
    if (recipeData) {
      try { return JSON.parse(recipeData); } catch (e) { return null; }
    }
    return null;
  }, [recipeData]);
  ...
  // Animation for 2D Floating Label
  ...
  const floatInterpolation = floatAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [0, -20], // Float up 20 pixels
  });
  ...
  useEffect(() => {
    const initSession = async () => {
      // Very crude simulation of session init
      setIsReady(true);
    };
    initSession();
  }, []);
    ...
    try {
      if (recipe.ingredients) {
        for (const ing of recipe.ingredients) {
          // Determine normalized name for matching since consumeIngredient uses semantic matching internally
          // but we pass raw name too.
          await consumeIngredients(ing.amount, ing.unit, ing.name);
        }
      }
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      Alert.alert("Inventory Updated", "Ingredients have been deducted from your pantry.");
    } catch (e) {
  ...
  if (!permission?.granted) {
    return (
      <View style={[styles.container, { paddingTop: insets.top, justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={{ color: 'white', marginBottom: 20 }}>Camera permission is required for AR Cooking.</Text>
  ...
  if (showCompletion) {
    return (
      <View style={styles.container}>
        <View style={[styles.completionOverlay, { paddingTop: insets.top, paddingHorizontal: 20 }]}>
          <Award size={64} color={Colors.primary} />
          <Text style={styles.completionTitle}>Cooking Complete!</Text>

          <TouchableOpacity
            style={[styles.btn, { marginBottom: 15, backgroundColor: isConsuming ? '#555' : Colors.primary }]}
            onPress={handleFinishCooking}
            disabled={isConsuming}
          >
            <Text style={styles.btnText}>{isConsuming ? "Updating Pantry..." : "Finish & Deduct Ingredients"}</Text>
          </TouchableOpacity>

          <TouchableOpacity style={[styles.btn, { backgroundColor: 'transparent', borderWidth: 1, borderColor: 'white' }]} onPress={handleSkipDeduction}>
            <Text style={[styles.btnText, { color: 'white' }]}>Skip Deduction</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
  ...
  return (
    <View style={styles.container}>
      ...
      <CameraView style={StyleSheet.absoluteFill} facing="back">
        ...
        {/* Layer 2: 2D AR Overlay (Fallback) */}
        ...
        <View style={styles.uiLayer}>
          <View style={[styles.header, { paddingTop: insets.top + 16 }]}>
            ...
            <View style={styles.progressContainer}>
              <Text style={styles.stepTitle}>Step {step.id} / {steps.length}</Text>
              <View style={styles.progressBar}>
                <View style={{ width: `${((currentStep + 1) / steps.length) * 100}%`, height: '100%', backgroundColor: Colors.primary }} />
              </View>
            </View>
          </View>
          ...
          <View style={[styles.footer, { paddingBottom: insets.bottom + 20 }]}>
            <TouchableOpacity style={styles.micBtn} onPress={toggleListening}>
              <Mic size={28} color={isListening ? Colors.primary : "white"} />
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  stepTitle: { color: 'white', fontWeight: 'bold', fontSize: 16, marginBottom: 5 },
  progressBar: { height: 4, backgroundColor: 'rgba(255,255,255,0.3)', borderRadius: 2, overflow: 'hidden' },
  footer: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 30 },
  micBtn: { width: 60, height: 60, borderRadius: 30, backgroundColor: 'rgba(0,0,0,0.5)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)' },
  nextBtn: { flexDirection: 'row', backgroundColor: Colors.primary, paddingVertical: 15, paddingHorizontal: 30, borderRadius: 30, alignItems: 'center', gap: 10 },
  nextText: { color: Colors.backgroundDark, fontWeight: 'bold', fontSize: 18 },
  completionOverlay: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: Colors.backgroundDark },
  completionTitle: { color: 'white', fontSize: 28, fontWeight: 'bold', marginVertical: 20 },
  btn: { backgroundColor: Colors.primary, paddingVertical: 15, paddingHorizontal: 40, borderRadius: 30 },
  btnText: { color: Colors.backgroundDark, fontWeight: 'bold', fontSize: 16 }
});
